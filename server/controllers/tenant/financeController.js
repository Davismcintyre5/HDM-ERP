...
const deleteAccount = async (req, res) => {
  try {
    await Account.findOneAndDelete({ _id: req.params.id, tenantId: req.tenantId });
...
const createJournal = async (req, res) => {
  try {
    ...
    const entry = await JournalEntry.create({
      tenantId: req.tenantId, entryNumber: `JE-${Date.now()}`, date, description, reference, lines,
      totalDebit, totalCredit, status: 'posted', source: 'manual', createdBy: req.user._id
    });

    // Update account balances
    for (const line of lines) {
      await Account.findOneAndUpdate(
        { _id: line.account, tenantId: req.tenantId },
        { $inc: { currentBalance: (line.debit || 0) - (line.credit || 0) } }
      );
    }
    ...
    if (status === 'paid') {
      ...
      if (cashAccount && revenueAccount) {
        const entry = await JournalEntry.create({
          ...
          description: `Payment for ${invoice.invoiceNumber}`, lines: [
            { account: cashAccount._id, debit: invoice.grandTotal, credit: 0, description: 'Cash receipt' },
            { account: revenueAccount._id, debit: 0, credit: invoice.grandTotal, description: 'Revenue' }
          ], totalDebit: invoice.grandTotal, totalCredit: invoice.grandTotal, status: 'posted', source: 'invoice', sourceId: invoice._id
        });
  ...
  try {
    ...
    if (status === 'paid') {
      ...
      if (cashAccount && expenseAccount) {
        const entry = await JournalEntry.create({
          tenantId: req.tenantId, entryNumber: `JE-BILL-${bill.billNumber}`, date: new Date(),
          description: `Payment for ${bill.billNumber}`, lines: [
            { account: expenseAccount._id, debit: bill.grandTotal, credit: 0, description: 'Expense' },
            { account: cashAccount._id, debit: 0, credit: bill.grandTotal, description: 'Cash payment' }
          ], totalDebit: bill.grandTotal, totalCredit: bill.grandTotal, status: 'posted', source: 'bill', sourceId: bill._id
        });
  ...
  try {
    ...
    if (cashAccount && incomeAccount) {
      await JournalEntry.create({
        tenantId: req.tenantId, entryNumber: `JE-REV-${Date.now()}`, date: date || new Date(),
        description: `Revenue from ${payerPayee}`, lines: [
          { account: cashAccount._id, debit: amount, credit: 0 },
          { account: incomeAccount._id, debit: 0, credit: amount }
        ], totalDebit: amount, totalCredit: amount, status: 'posted', source: 'other', sourceId: payment._id
      });
    ...
    payment.journalEntry = /* populated later if needed */ undefined;
...
const recordExpense = async (req, res) => {
  try {
    ...
    if (cashAccount && expenseAccount) {
      await JournalEntry.create({
        tenantId: req.tenantId, entryNumber: `JE-EXP-${Date.now()}`, date: date || new Date(),
        description: `Expense to ${payerPayee}`, lines: [
          { account: expenseAccount._id, debit: amount, credit: 0 },
          { account: cashAccount._id, debit: 0, credit: amount }
        ], totalDebit: amount, totalCredit: amount, status: 'posted', source: 'other', sourceId: payment._id
      });
      await Account.findByIdAndUpdate(cashAccount._id, { $inc: { currentBalance: -amount } });
