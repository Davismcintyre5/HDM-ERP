...
const ReportsPage = () => {
  ...
  const handlePrint = () => {
    ...
    if (Array.isArray(viewData)) {
      ...
      printTable(viewData, keys.map(k => ({ key: k, label: k.replace(/([A-Z])/g, ' $1').replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase()) })), { title: 'Report Results' });
    } else {
      import('../../utils/printUtils').then(m => m.printDetail(viewData, { title: 'Report Results' }));
    }
  };
            ...
            {Array.isArray(viewData) && viewData.length > 0 ? (
              <div className="overflow-x-auto">
                <table className="w-full text-xs">
                  <thead><tr className="border-b border-gray-200 dark:border-gray-700">{Object.keys(viewData[0]).map(k => <th key={k} className="py-2 px-2 text-left text-gray-500 dark:text-gray-400 font-medium whitespace-nowrap">{k.replace(/([A-Z])/g, ' $1').replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase())}</th>)}</tr></thead>
                  <tbody>{viewData.map((row, i) => <tr key={i} className="border-b border-gray-100 dark:border-gray-700/50">{Object.values(row).map((v, j) => <td key={j} className="py-2 px-2 text-gray-700 dark:text-gray-300 whitespace-nowrap">{formatValue(v)}</td>)}</tr>)}</tbody>
                </table>
              </div>
            ) : (
