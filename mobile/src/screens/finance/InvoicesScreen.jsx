...
const emptyForm = { customerName: '', invoiceDate: new Date().toISOString().split('T')[0], dueDate: '', items: [{ description: '', quantity: 1, unitPrice: 0, taxRate: 0 }], notes: '' };
...
const InvoicesScreen = ({ navigation }) => {
  ...
  const fetchInvoices = useCallback(async () => {
    try { const res = await getInvoices(); setInvoices(res.data.data || []); } catch {} finally { setLoading(false); }
  }, []);
  ...
  const handleCreate = async () => {
    ...
    setSaving(true);
    try { await createInvoice(form); setShowForm(false); setForm(emptyForm); fetchInvoices(); } catch (err) { setMessage(err.response?.data?.message || 'Failed'); } finally { setSaving(false); }
  };
  ...
  const advanceStatus = async (id, current) => {
    ...
    if (idx < flow.length - 1) {
      try { await updateInvoiceStatus(id, flow[idx + 1]); fetchInvoices(); } catch {}
    }
  };
  ...
  return (
    <View style={styles.flex}>
      <HeaderBar title="Invoices" onBack={() => navigation.goBack()} />
          ...
          renderItem={({ item }) => (
            <TouchableOpacity style={styles.row} onPress={() => advanceStatus(item._id, item.status)} activeOpacity={0.7}>
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.invCustomer}>{item.customerName || 'N/A'}</Text>
                <Text style={styles.invDate}>{item.invoiceDate ? new Date(item.invoiceDate).toLocaleDateString() : ''}</Text>
              </View>
              <View style={{ alignItems: 'flex-end', gap: 4 }}>
                <Badge variant={item.status === 'paid' ? 'success' : item.status === 'sent' ? 'info' : 'default'}>{item.status}</Badge>
      ...
      <Modal open={showForm} onClose={() => setShowForm(false)} title="New Invoice">
        <Input label="Customer Name *" value={form.customerName} onChangeText={(v) => setForm(p => ({ ...p, customerName: v }))} placeholder="Customer name" required />
        <View style={{ flexDirection: 'row', gap: 8 }}>
          <Input label="Invoice Date" value={form.invoiceDate} onChangeText={(v) => setForm(p => ({ ...p, invoiceDate: v }))} style={{ flex: 1 }} />
        ...
        <Button title="+ Add Item" variant="ghost" size="sm" onPress={() => setForm(p => ({ ...p, items: [...p.items, { description: '', quantity: 1, unitPrice: 0, taxRate: 0 }] }))} />
        <Text style={styles.total}>
          Total: {formatCurrency(form.items.reduce((s, i) => s + (i.quantity * i.unitPrice), 0))}
        </Text>
        <Input label="Notes" value={form.notes} onChangeText={(v) => setForm(p => ({ ...p, notes: v }))} placeholder="Optional" />
...
const styles = StyleSheet.create({
  ...
  invCustomer: { fontSize: 13, color: '#6B7280', marginTop: 2 },
  invDate: { fontSize: 11, color: '#9CA3AF', marginTop: 2 },
  invAmount: { fontSize: 15, fontWeight: '700', color: '#10B981' },
