...
const emptyForm = { customer: '', customerName: '', orderDate: new Date().toISOString().split('T')[0], items: [], notes: '' };
...
const OrdersScreen = ({ navigation }) => {
  ...
  const fetchOrders = useCallback(async () => {
    try {
      const [oRes, cRes, pRes] = await Promise.all([getOrders(), getContacts(), getProducts()]);
  ...
  const addItem = () => setForm(p => ({ ...p, items: [...p.items, { product: '', quantity: 1, unitPrice: 0, discount: 0, taxRate: 0 }] }));
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      await createOrder({ ...form, customer: existing ? form.customer : undefined, customerName: existing ? undefined : form.customerName });
      setShowForm(false); setForm(emptyForm); fetchOrders();
  ...
  const advanceStatus = async (id, current) => {
    ...
    if (idx < statusFlow.length - 1) {
      try { await updateOrderStatus(id, statusFlow[idx + 1]); fetchOrders(); } catch {}
    }
  };
    ...
    <View style={styles.flex}>
      ...
      <ScreenWrapper scrollable={false}>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <TouchableOpacity style={styles.row} onPress={() => advanceStatus(item._id, item.status)} activeOpacity={0.7}>
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.orderCustomer}>{item.customer?.companyName || item.customerName || 'N/A'}</Text>
                <Text style={styles.orderDate}>{formatDate(item.orderDate)}</Text>
              </View>
              <View style={{ alignItems: 'flex-end', gap: 4 }}>
                <Badge variant={statusColors[item.status] || 'default'}>{item.status}</Badge>
                <Text style={styles.orderAmount}>{formatCurrency(item.grandTotal)}</Text>
          ...
          ListEmptyComponent={<EmptyState icon="cart-outline" title="No sales orders" />}
        />
      </ScreenWrapper>
      ...
      <Modal open={showForm} onClose={() => setShowForm(false)} title="New Sales Order">
        <Select
          label="Customer"
          value={form.customer}
          onValueChange={(v) => setForm(p => ({ ...p, customer: v, customerName: '' }))}
          options={contacts.map(c => ({ label: c.companyName, value: c._id }))}
          placeholder="Select customer"
        />
        <Input label="Or New Customer" value={form.customerName} onChangeText={(v) => setForm(p => ({ ...p, customerName: v, customer: '' }))} placeholder="Type new customer name" />
        <Input label="Order Date" value={form.orderDate} onChangeText={(v) => setForm(p => ({ ...p, orderDate: v }))} />
        ...
        {form.items.map((item, idx) => (
          <View key={idx} style={styles.itemRow}>
            <Select
              ...
              options={products.map(p => ({ label: `${p.name} (${formatCurrency(p.sellingPrice)})`, value: p._id }))}
              placeholder="Product"
              style={{ flex: 2 }}
            />
            ...
            {form.items.length > 1 && (
              <TouchableOpacity onPress={() => removeItem(idx)}><Text style={{ color: '#EF4444', fontSize: 18 }}>✕</Text></TouchableOpacity>
            )}
          </View>
        ))}
        <Button title="+ Add Item" variant="ghost" size="sm" onPress={addItem} />
        <Text style={styles.total}>
          Total: {formatCurrency(form.items.reduce((s, i) => s + (i.quantity * i.unitPrice - (i.discount || 0)), 0))}
        </Text>
        ...
        <View style={{ flexDirection: 'row', gap: 10, marginTop: 16 }}>
          ...
          <Button title="Create Order" onPress={handleCreate} loading={saving} style={{ flex: 1 }} />
        </View>
      </Modal>
    </View>
  );
